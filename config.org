...
** Theme
   ...
   (spaceline-define-segment git
     ...
     (when vc-mode
       (powerline-raw
        (concat (rst/inherit-icon (all-the-icons-octicon "git-compare" :v-adjust 0.01))
                " "
...
** Helpers
   ...
   Cycle through powerline separators

   #+begin_src emacs-lisp
   (defun rst/cycle-powerline-separator ()
     (interactive)
     (let* ((all-separators
             '("alternate"
               "arrow"
            ...
            (current-separator-index
             (cl-position powerline-default-separator all-separators :test 'equal))
            (next-separator-index
             (mod (+ current-separator-index 1) (length all-separators)))
            (next-separator
             (nth next-separator-index all-separators)))
       (setq powerline-default-separator next-separator)

       (spaceline-compile)
       (force-mode-line-update)
       (message powerline-default-separator)))
   #+end_src
