...
* General
  ...
  #+begin_src emacs-lisp
  (setenv "PATH" (concat (getenv "PATH") ":/usr/local/bin:/Library/TeX/texbin:/usr/local/go/bin:/opt/homebrew/bin/"))
  (setq exec-path (append exec-path '("/usr/local/bin" "/Library/TeX/texbin" "/usr/local/go/bin" "/opt/homebrew/bin")))
  #+end_src
...
** Debugging
   ...
   (defun rst/eval-debug-template-file ()
     "Evaluate debug template file so that it is available. This function can be
   called on .dir-locals.el ."
     (interactive)
     (let* ((current-project (projectile-project-root))
            (project-path (projectile-ensure-project current-project))
            (full-path (concat project-path rst/dap-debug-template-filename)))
       (with-temp-buffer
   ...
   (add-hook 'projectile-after-switch-project-hook #'rst/eval-debug-template-file)
   #+end_src
...
** Git
   ...
   #+begin_src emacs-lisp
   (defvar rst/base-branch "master"
     "Base branch to compare feature branches with.")

   (defun rst/diff-current-with-base-branch ()
     "Show diff between current branch and base branch"
     (interactive)
     (magit-diff-range (concat
                        rst/base-branch
                        ".."
...
* Keybindings
  ...
  (use-package general
    ...
    (bind-leader-key
      ...
      "D" '(dap-ui-controls-mode :which-key "toggle dap ui controls")

      ;; Used for editing and navigating saved helm ag session
      "f" '((lambda ()
              (interactive)
              (fundamental-mode)
              (read-only-mode -1)) :which-key "toggle fundamental mode")

      "g" '(helm-ag-mode :which-key "toggle helm ag mode")
    ...
    (bind-leader-key
      ...
      "d" '(rst/diff-current-with-base-branch :which-key "diff branch with base"))
    ...
    (bind-leader-key
      ...
      "F" '(rst/eval-debug-template-file :which-key "eval project debug template")
      ...
      "i" '(dap-next :which-key "next")
      "s" '(dap-step-in :which-key "step in")
      "S" '(dap-step-out :which-key "step out")
      "q" '(dap-disconnect :which-key "quit"))
    )

  (general-define-key :keymaps 'helm-ag-mode-map
                      "<return>" 'helm-ag-mode-jump
                      "<M-return>" 'helm-ag-mode-jump-other-window)
